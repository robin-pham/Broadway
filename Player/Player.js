...
module.exports = (function() {
  ...
  var Player = function(parOptions){
    ...
    if (this._config.useWorker){
      self.worker = new Worker(this._config.workerFile);
      ...
      self.worker.onerror = function(e){
        if(self._config.onError) {
          self._config.onError(e);
        }
      };

      self.worker.postMessage({type: "Broadway.js - Worker init", options: {
        rgb: !webgl,
      ...
      if (this._config.transferMemory){
        this.decode = function(parData, parInfo){
          ...
          self.worker.postMessage({buf: parData.buffer, offset: parData.byteOffset, length: parData.length, info: parInfo}, [parData.buffer]); // Send data to our worker.
        };
        ...
        this.decode = function(parData, parInfo){
          ...
          self.worker.postMessage({buf: copyU8.buffer, offset: 0, length: parData.length, info: parInfo}, [copyU8.buffer]); // Send data to our worker.
        };
      ...
      if (this._config.reuseMemory){
        this.recycleMemory = function(parArray){
          //this.beforeRecycle();
          self.worker.postMessage({reuse: parArray.buffer}, [parArray.buffer]); // Send data to our worker.
          //this.afterRecycle();
        };
      }
